...
export function WelcomeBody(props: WelcomeBodyProps) {
  const { isOpen } = props;

  const { t } = useTranslation("welcomeBody");

  const { dmOptions, filesDatabase, fileStateManager } =
  ...
  const [recordToDelete, setRecordToDelete] =
    useState<FilesDatabaseRecord | null>(null);

  // uploading state
  const [fileToUpload, setFileToUpload] = useState<AppFile | null>(null);
  const [fileToOverwrite, setFileToOverwrite] = useState<AppFile | null>(null);
  ...
  async function handleFileUpload(input) {
    ...
    input.value = null;
    const uploadedAppFile = AppFile.fromJson(
      dmOptions,
  ...
  if (!isOpen) {
    return (
      <div
        className={`${styles["welcome-body"]} ${isOpen ? "" : styles["welcome-body--closed"]}`}
      ></div>
    );
  }
      ...
      <div className={styles["centering-container"]}>
        <Paper className={styles["paper"]} square>
              ...
              <Table size="small">
                <TableHead>
                  <TableRow>
                    ...
                    <TableCell>
                      {t("browserFiles.column.lastModified")}
                    </TableCell>
                ...
                <TableBody>
                  {fileList.map((record) => (
                    <TableRow key={record.uuid}>
                      ...
                      <TableCell>
                        <Tooltip
                          title={t("browserFiles.tip.lastModified", {
                            timestamp: record.updatedAt,
                          })}
                          disableInteractive
                        >
                          <span>
                            {t("browserFiles.row.lastModified", {
                              timestamp: record.updatedAt,
                            })}
                          </span>
                        </Tooltip>
                      ...
                      <TableCell>
                        <Tooltip
                          title={t("browserFiles.tip.createdAt", {
                            timestamp: record.createdAt,
                          })}
                          disableInteractive
                        >
                          <span>
                            {t("browserFiles.row.createdAt", {
                              timestamp: record.createdAt,
                            })}
                          </span>
                        </Tooltip>
                      ...
                      <TableCell align="right">
                        <Tooltip
                          title={t("browserFiles.tip.delete")}
                          disableInteractive
                        >
                          <IconButton
                            size="small"
                        ...
                        <Tooltip
                          title={t("browserFiles.tip.download")}
                          disableInteractive
                        >
                          <IconButton
                            size="small"
                            className={styles["file-list__action-button"]}
                            onClick={() =>
                              filesDatabase.downloadFile(record.uuid)
                            }
                          >
          ...
          <Stack
            direction="row"
            spacing={2}
            className={styles["new-file-actions"]}
          >
            <Button
              variant="contained"
          ...
          <Typography
            variant="body2"
            gutterBottom
            sx={{ color: "#888", paddingTop: 1 }}
          >
            {dmOptions.customization.name} v{dmOptions.customization.version}
            <br />
            DocMarker v{VERSION}
          </Typography>
          ...
          >
            ...
            <DialogContent>
              ...
              <Typography variant="body2" gutterBottom>
                {recordToDelete
                  ? t("deleteDialog.lastModified", {
                      timestamp: recordToDelete.updatedAt,
                    })
                  : ""}
              </Typography>
            </DialogContent>
          ...
          <Dialog open={fileToUpload !== null} onClose={closeUploadFileDialog}>
            ...
            <DialogContent>
              ...
              <Typography variant="body2" gutterBottom>
                {fileToUpload
                  ? t("uploadDialog.lastModified", {
                      timestamp: fileToUpload.updatedAtString,
                    })
                  : ""}
              </Typography>
              <div style={{ height: "20px" }}></div>
              ...
              <Typography variant="body2" gutterBottom>
                {fileToOverwrite
                  ? t("uploadDialog.lastModified", {
                      timestamp: fileToOverwrite.updatedAtString,
                    })
                  : ""}
              </Typography>
            </DialogContent>
            <DialogActions>
              <Button variant="outlined" onClick={closeUploadFileDialog}>
