...
import { QAttributes } from "./QDelta";
 ...
 */
export function getInlineFormatRange(
  quill: Quill,
  index: number,
  formatName: string,
  formatValue: any,
...
function followFormatLeft(
  ...
  while (true) {
    if (quill.getText(index, 1) != "\n") {
      const format: QAttributes = quill.getFormat(index, 1);
      if (format[formatName] !== formatValue) break;
    }
    index -= 1;
  }
  return index + 1;
...
function followFormatRight(
  ...
  while (true) {
    if (quill.getText(index, 1) != "\n") {
      let format: QAttributes = quill.getFormat(index, 1);
      if (format[formatName] !== formatValue) break;
    }
    index += 1;
  }
